 ...
 * 合并样式对象
 */
const mergeStyles = (...styles: (ToastStyleOptions | undefined)[]): ToastStyleOptions | undefined => {
  const merged = Object.assign({}, ...styles.filter((style): style is ToastStyleOptions => !!style));
...
const buildOptions = (options?: ToastOptions, preset?: PresetType): ToastOptions => {
  ...
  const mergedStyle = mergeStyles(config.defaultOptions.style, presetStyle, options?.style);
...
const configure = (partial: ToastConfig): void => {
  if (partial.defaultOptions) {
    ...
    if (Object.prototype.hasOwnProperty.call(partial.defaultOptions, 'style')) {
      NativeToaster.setDefaultStyle(partial.defaultOptions.style ?? null);
    }
  }
